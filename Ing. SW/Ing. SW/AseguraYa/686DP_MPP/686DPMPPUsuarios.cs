...
namespace _686DP_MPP
{
    public class _686DPMPPUsuarios
    {
        ...
        public void _686DPActualizarEmpleadoExistente(_686DP_Empleados emp)
        {
            try
            {
                string consulta = @"UPDATE [dbo].[686DP_Empleado] SET 
                                DP686_Nombre = @Nombre,
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow item in dt.Rows)
                    {
                        try
                        {
                            ...
                            _686DP_Empleados empleado = new _686DP_Empleados(DNI, Nombre, Apellido, Email, Rol, usuario, contraseña, Activo, Bloqueado);
